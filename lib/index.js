...
class Deploy extends events_1.default {
    ...
    async build() {
        const { sourceDir, containerName } = this;
        const pkg = require(path_1.default.join(sourceDir, 'package.json'));
        const buildOptions = pkg.build || pkg.cloudBuild;
        if (!buildOptions || !buildOptions.script) {
            return;
        }
        const engine = buildOptions.engine || this.engine || 'latest';
        const registry = buildOptions.registry || 'https://registry.dingxiang-inc.net';
        const install = [
            buildOptions.yarn ? 'yarn' : 'npm i',
            `--registry=${registry}`
        ].join(' ');
        const cmd = `
      docker run \
      ...
      -w /var/www \
      node:${engine} \
      sh -c '${install} && ${buildOptions.script}'
    `;
        await this.exec(cmd);
    }
    async sync() {
        ...
        await util_1.link(path_1.default.relative(path_1.default.dirname(target), actual), target.split(path_1.default.sep).pop());
